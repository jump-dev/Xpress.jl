...
function lpoptimize(model::Model, flags::String="")
    ...
    model.time = @elapsed ret = @xprs_ccall(lpoptimize, Cint, (Ptr{Cvoid},Ptr{UInt8}),
...
function mipoptimize(model::Model, flags::String="")
    ...
    model.time = @elapsed ret = @xprs_ccall(mipoptimize, Cint, (Ptr{Cvoid},Ptr{UInt8}),
        model.ptr_model, flags)
...
function computeIIS(model::Model)
    @assert model.ptr_model != C_NULL
    ret = @xprs_ccall(iisall, Cint, (Ptr{Cvoid},), model.ptr_model)
    if ret != 0
        throw(XpressError(model))
    end
    nothing
...
function loaddelayedrows(model::Model, mrows::Array{I} where I <: Integer)
    nrows = length(mrows)
    @assert model.ptr_model != C_NULL
    ret = @xprs_ccall(loaddelayedrows, Cint, (Ptr{Cvoid},Cint,Ptr{Cint}),
